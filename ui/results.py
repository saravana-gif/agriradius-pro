...
def _data_confidence_panel():
    ...
    with st.expander("ℹ️ Data & Confidence - how much to trust this",
                     expanded=False):
        st.markdown(
            "**Measured on the ground - the strongest thing here:**\n"
            "- **Soil Health Card lab results** - real farmer samples "
            "tested in government labs, village by village\n"
            "- **Karnataka Crop Survey (coconut)** - every plot logged "
            "against its survey number with GPS, 3,318 villages in 6 "
            "districts. Not an estimate at all\n"
            "- **Irrigation by source** (canal / tank / borewell / dug "
            "well) - Land Use Statistics 2022-23, all 31 Karnataka "
            "districts\n"
            "- **Mandi prices** - reported by markets\n"
            "- Your team's own **ground-truth** observations\n\n"
            "**High confidence (direct satellite measurement):**\n"
            ...
            "structure\n"
            "- **Radar irrigation events** - Sentinel-1 backscatter "
            "rise; the only method here that works under cloud "
            "(~86% discrimination in published work)\n\n"
            ...
            "- **Satellite irrigation** - 80-90% at parcel level in "
            "the semi-arid interior and north, but only **60-75% on "
            "the coast and in Malnad**, where year-round rain hides "
            "the signal. The app tells you which zone you are in\n"
            "- **LGRIP30** irrigated/rain-fed - its 91% accuracy is "
            "**continental-US only**; India is V001 (2015) with no "
            "published Indian accuracy, and rain-fed user's accuracy "
            "in South Asia is only 63%\n"
            "- **WorldCereal irrigation** - treat as a **lower bound**: "
            "no published accuracy metrics, under-maps Asia\n"
            "- Cropping-pattern & sourcing-score thresholds\n\n"
            "**Built-in cross-checks:**\n"
            "- Two independent cropland datasets (Dynamic World + ESA "
            "WorldCover/WorldCereal) compared, plus a 3-year stability "
            "test\n"
            "- **Satellite plantation vs the coconut crop survey** - "
            "measured acreage against detected acreage for the same "
            "circle\n"
            "- **Irrigation is scored 0-5** by how many independent "
            "methods agree (summer greenness, radar events, "
            "multi-cropping, LGRIP30, WorldCereal). The app quotes the "
            "agreement figure, never a single product\n"
            "- **Satellite irrigation vs the survey's own irrigation "
            "flags** for ~3,300 villages\n\n"
            "**Two traps this app deliberately avoids:**\n"
            "- *Rain-fed rabi:* jowar, chickpea and safflower on black "
            "cotton soil (vertisols) in Vijayapura, Bagalkote, "
            "Kalaburagi, Bidar and Vijayanagara are **rain-fed** on "
            "stored soil moisture. A 'green in rabi = irrigated' rule "
            "mislabels much of north Karnataka, so the irrigation "
            "layer uses the **February-May** window instead\n"
            "- *Invisible borewells:* 56.6% of Karnataka's irrigated "
            "area is borewell-fed and appears on **no** canal "
            "command-area map. The app infers it from distance to "
            "permanent surface water instead\n\n"
            "**Honest limit:** no government or commercial product "
            "gives a reliable, current, **plot-level** irrigated / "
            "rain-fed flag for all of Karnataka. Only the RTC/Pahani "
            "(authoritative for land class, stale for new borewells) "
            "and the seasonal Crop Survey come close, and neither has "
            "an open bulk API.\n\n"
            ...
            "WorldCover/WorldCereal (ESA), LGRIP30 & GCEP30 "
            "(USGS/NASA), GCI30, SoilGrids (ISRIC), CHIRPS (UCSB), "
            "ERA5-Land (ECMWF), JRC Global Surface Water, Open-Meteo, "
            "AGMARKNET (data.gov.in), Soil Health Card scheme, "
            "Karnataka Crop Survey, Land Use Statistics (DES-Agri), "
            "India-WRIS, Livestock Census 2019, SLUSI & CGWB (GoI).*"
        )
